...
fn start_cli(app: AppHandle) {
    ...
    *REFRESH_CLI.lock().unwrap() = Some(sender);
    ...
    std::thread::spawn(move || {
        ...
        loop {
            let explicit = receiver.recv_timeout(Duration::from_secs(60)).is_ok();
            if explicit {
                while receiver.try_recv().is_ok() {}
            }

            if explicit && last_attempt.is_some_and(|last| last.elapsed() < Duration::from_secs(5)) {
                continue;
            }

            let st = app.state::<AppState>();
            let previous = st.antigravity.lock().unwrap().clone();
            if !explicit && !previous.windows.is_empty()
                && now_ms().saturating_sub(previous.fetched_at) < 60_000
            {
                continue;
            }
