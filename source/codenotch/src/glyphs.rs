...
/// Built-in artwork (@lobehub/icons-static-svg, MIT / upstream vector artwork)
const BUILTIN: [(&str, &str); 11] = [
    ...
    ("gemini_api", include_str!("../glyphs/gemini-spark.svg")),
    ("claude_api", include_str!("../glyphs/claude.svg")),
    ("copilot", include_str!("../glyphs/copilot.svg")),
    ("ollama", include_str!("../glyphs/ollama.svg")),
    ("nvidia", include_str!("../glyphs/nvidia.svg")),
];
...
fn app_candidates(id: &str) -> Vec<PathBuf> {
    ...
    match id {
        ...
        "ollama" => {
            v.push(programs.join("Ollama").join("ollama.exe"));
        }
        _ => {}
    }
